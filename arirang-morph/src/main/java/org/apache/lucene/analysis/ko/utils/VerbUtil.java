...
public class VerbUtil {
  ...
  public static boolean ananlysisNJCM(AnalysisOutput o, List<AnalysisOutput> candidates) throws MorphException {
    ...
    //에서 -> 서 축약형을 위한 파트
    if(success&&o.getStem().endsWith("서")&&entry==null) {
    	int seolength = o.getStem().length();
    	entry=DictionaryUtil.getAllNoun(o.getStem().substring(0, seolength-1));
    	if(entry!=null) {
    		o.setJosa(o.getStem().substring(seolength-1)+o.getJosa());
    		o.setStem(o.getStem().substring(0, seolength-1));
    		o.setScore(AnalysisOutput.SCORE_CORRECT);
    	}
    }
    ...
    if(entry==null) {
    	...
    	if(entry.getFeature(WordEntry.IDX_NOUN)!='0') {
    		...
    		if(o.getVsfx().equals("하")) {
    			if(entry.getFeature(WordEntry.IDX_DOV)=='0') return false;
    			else o.setUsedPosType(entry.getFeature(WordEntry.IDX_DOV));
    		} else if(o.getVsfx().equals("되")) {
    			if(entry.getFeature(WordEntry.IDX_BEV)=='0') return false;
    			else o.setUsedPosType(entry.getFeature(WordEntry.IDX_BEV));
    		} 
